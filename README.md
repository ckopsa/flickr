...
18. **Every silent file gets subtitles** — the one thing no shop-bought box
   ...
   A box that would rather not spend its own CPU hands the work out instead:
   `GET /api/transcripts` is the same rule said as a queue a worker on another
   node reads — the silent files, each with a presigned URL and the
...
| **transcripts** | `GET /api/transcripts` | the work a transcription worker elsewhere reads: `queue` — up to `limit` (20, max 100) files carrying no subtitle track of their own and no transcript at this etag, oldest id first, each with `object_key`, `etag`, `medium`, `duration_seconds`, a presigned `url` and `links.item` — `count`, the whole backlog, and `leased`, how many are out. Reading the queue HANDS its entries out (an hour each), so two workers do not hear the same film; `?all=1` reads without taking | per entry `transcript`: `POST /api/items/{id}/transcript`, the WebVTT as the body and `etag` on the query or an `X-Transcript-ETag` header, answering `{item_id, cues, language, model}` | `bad-item-id`, `missing-etag`, `bad-language` 400, `no-such-item` 404, `has-subtitles`, `stale-transcript` 409, `transcript-too-large` 413, `empty-transcript` 422 |
